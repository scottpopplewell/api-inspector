function clearTableData() {
  document.getElementById('restAPIBody').innerHTML = '';
}
...
function loadTableData(item) {
  ...
  // Splunk Cell
  let debugUrls = row.insertCell(5);
  const environment = document.querySelector('input[name="environment"]:checked').value;
  debugUrls.innerHTML = environment === "QA" ? "<a target='_blank' href='" + item.debugQAUrl + "'>QA Logs</a>"
                        : "<a target='_blank' href='" + item.debugProdUrl + "'>Prod Logs</a>"
}
